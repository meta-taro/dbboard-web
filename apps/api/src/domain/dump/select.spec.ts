...
describe("buildSelectPage", () => {
  it("reads one unordered page when the table has no key", () => {
    expect(buildSelectPage(users, [], 1000)).toBe(`SELECT * FROM "public"."users" LIMIT 1000`);
  });

  it("orders by the key on the first page", () => {
    expect(buildSelectPage(users, ["id"], 1000)).toBe(
      `SELECT * FROM "public"."users" ORDER BY "id" LIMIT 1000`,
    );
  });

  it("adds a keyset cursor from the second page on", () => {
    expect(buildSelectPage(users, ["id"], 1000, [42])).toBe(
  ...
  it("compares a composite key as a row value", () => {
    ...
    expect(buildSelectPage(users, ["tenant", "id"], 500, ["acme", 7])).toBe(
  ...
  it("ignores a cursor when there is no key to order by", () => {
    expect(buildSelectPage(users, [], 10, [1])).toBe(`SELECT * FROM "public"."users" LIMIT 10`);
  });

  it("omits the schema when the table is unqualified", () => {
    expect(buildSelectPage(unqualified, [], 10)).toBe(`SELECT * FROM "users" LIMIT 10`);
  });

  it("quotes key identifiers", () => {
    expect(buildSelectPage(unqualified, ['i"d'], 10)).toBe(
      `SELECT * FROM "users" ORDER BY "i""d" LIMIT 10`,
    );
  });

  it("refuses a cursor whose arity does not match the key", () => {
    expect(() => buildSelectPage(users, ["a", "b"], 10, [1])).toThrow(CursorError);
  });
  ...
  it("refuses a null cursor value rather than silently truncating the dump", () => {
    ...
    expect(() => buildSelectPage(users, ["id"], 10, [null])).toThrow(CursorError);
  });
});

describe("buildCount", () => {
  it("counts the qualified table", () => {
    expect(buildCount(users)).toBe(`SELECT COUNT(*) FROM "public"."users"`);
  });

  it("counts an unqualified table", () => {
    expect(buildCount(unqualified)).toBe(`SELECT COUNT(*) FROM "users"`);
  });
});
