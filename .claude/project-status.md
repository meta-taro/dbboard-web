...
**Phase 2 / 3 closeout (`0005`) — code complete on `feature/phase-3-conformance` (2026-06-04).** Domain constants `ROW_CAP = 10_000` and `QUERY_BODY_LIMIT_BYTES = 64 * 1024` in `apps/api/src/domain/limits.ts`; row cap enforced in `ExecuteQuery.execute` so no adapter can ship a truncated grid; body cap fed into `bootstrap/config.ts` for the `bodyParser` seam (`MAX_BODY_BYTES`). New `RequestLevelRejectionFilter` (`@Catch()`, catch-all) replaces Nest's default JSON envelope on 400 (malformed JSON), 413 (body-parser `PayloadTooLargeError`), and surfaces `ContractErrorFilter` first under Nest's LIFO global-filter resolution so the existing `CategorizedError` envelope still wins. Cross-implementation conformance battery under `apps/api/tests/conformance/` — 11 cases (health / tables / capabilities / SELECT 1 / value union / malformed JSON / missing sql / wrong content-type / oversize body / row-cap exceeded / divide-by-zero), skip-gated on `DBBOARD_SERVER_BIN` and a reachable Docker daemon; `pnpm conformance` (alias `pnpm --filter @dbboard-web/api conformance`) drives the suite. Compare helper unit-tested under the same config so the deep-equal / token-substring logic stays honest without a Rust toolchain on the dev box. 168 (api) + 90 (web) + 11 (conformance helper) green; PR not yet opened.
