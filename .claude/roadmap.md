...
- [x] Export endpoint streams `application/x-ndjson` records that round-trip via `jq -c .` against a desktop `history.jsonl` fixture. **[egress done, 0009-impl — `f8154c3`; cross-check scaffold + handoff brief landing 2026-06-23 under [`0018`](./issues/0018-history-export-roundtrip-fixture.md), bytes awaited from desktop agent — `apps/api/test/desktop-history-roundtrip.spec.ts` is `describe.skipIf(!existsSync(...))` and lights up automatically when `apps/api/test/fixtures/desktop-history.jsonl` lands]**
