...
> **Parallelizable with Phase 2 and Phase 3.** PWA work touches the application shell only — `manifest.json`, service worker, responsive layout, install prompt — and does not depend on the HTTP contract or any backend endpoint. It can land while the contract follow-ups (issues `0003`, `0004`, `0005`) are still waiting on the desktop's `/capabilities` publication. See incoming brief at [`handoff/2026-05-26-pwa-pivot-incoming.md`](./handoff/2026-05-26-pwa-pivot-incoming.md) and issue [`0006`](./issues/0006-pwa-shell.md).

**DoD**

- `apps/web/public/manifest.json` present with `name`, `icons` (192×192 and 512×512 maskable), `start_url`, `display: standalone`, and `theme_color`.
- Service worker auto-generated by a Nuxt PWA module (`@vite-pwa/nuxt`) with precache and an explicit offline fallback page.
- Install-prompt UI is opt-in (captures `beforeinstallprompt`, never auto-pushes).
- iOS Safari support: `apple-touch-icon`, `apple-mobile-web-app-capable`, and a status-bar style configured.
